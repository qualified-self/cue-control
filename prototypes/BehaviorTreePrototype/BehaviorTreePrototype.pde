...
//  frameRate(5);
  ...
  root = new SequentialNode("Wait for starting cue then launch")
                ...
                .addChild(new ParallelNode("Start the show", true, true)
                  .addChild(new OscToBlackboard("/cue-proto/start", "start", State.SUCCESS).setDecorator(new WhileDecorator(new BlackboardCondition("[end] == 0"))))
                  .addChild(new OscToBlackboard("/cue-proto/level", "level", State.SUCCESS).setDecorator(new WhileDecorator(new BlackboardCondition("[end] == 0"))))
